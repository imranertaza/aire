                ...
                <div class="col-lg-3 border-right1px pt-4 pe-lg-4">
                    ...
                    <div class="confirm-summary-card">
                            ...
                            @if ($order && $order->items)
                                @foreach ($order->items as $item)
                                    <div class="confirm-product-row">
                                        <img src="{{ getImageCacheUrl($item->product->main_image, 200, 200, 'webp') ?? theme_asset('img/Air-Purify.png') }}"
                                            alt="{{ $item->product->name ?? 'AIRE Pro S1' }}" class="confirm-product-thumb"
                                            style="object-fit: contain;">
                                        <div>
                                            <h3 class="confirm-product-name">{{ getLimitedText($item->product->name, 50) }}
                                            </h3>
                                            <p class="confirm-product-sub">{{ $item->product->model }}
                                            </p>
                        ...
                        <div class="confirm-row-item">
                            <span>Subtotal</span>
                            <span class="confirm-row-val">${{ number_format($order ? $order->total : 1299.0, 2) }}</span>
                        </div>
                        <div class="confirm-row-item">
                            <span>Shipping</span>
                        ...
                        <div class="confirm-row-item total">
                            <span>Total</span>
                            <span
                                class="confirm-row-val">${{ number_format($order ? $order->final_amount : 1299.0, 2) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-lg-6 pt-4 px-lg-4">
                    ...
                    <div class="confirm-hero-card">
                        ...
                        <p class="confirm-hero-desc">We've received your order and it's being processed.<br>A
                            ...
                            <strong>{{ $order ? $order->email : 'john.doe@example.com' }}</strong>.
                        </p>
                        <div class="confirm-order-pill">
                            <span class="confirm-order-pill-label">ORDER NUMBER</span>
                            AIRE-{{ $order ? $order->created_at->format('Y') : '2026' }}-{{ $order ? $order->invoice_no : '1234' }}
                        </div>
                    </div>
                    ...
                    <div class="confirm-card-box">
                        ...
                        <div class="confirm-timeline">
                            ...
                            <div class="confirm-timeline-step completed">
                                ...
                                <span
                                    class="confirm-timeline-sub">{{ $order ? $order->created_at->format('M d, Y') : 'Today' }}</span>
                            </div>
                    ...
                    <div class="confirm-card-box">
                        ...
                        <div class="confirm-detail-row">
                            ...
                            <span
                                class="confirm-detail-val">{{ $order ? $order->created_at->format('M d, Y') : 'Today' }}</span>
                        </div>
                        <div class="confirm-detail-row">
                            <span class="confirm-detail-label">PAYMENT METHOD</span>
                            <span class="confirm-detail-val">
                                <i class="bi bi-credit-card me-1"></i>
                                {{ $order ? $order->payment_method : 'Credit Card' }}
                            </span>
                        </div>
                        <div class="confirm-detail-row">
                            <span class="confirm-detail-label">STATUS</span>
                            <span
                                class="confirm-paid-badge">{{ $order ? strtoupper($order->payment_status) : 'PENDING' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="confirm-card-box">
                        ...
                        <div class="confirm-address-name">
                            {{ $order ? $order->shipping_firstname . ' ' . $order->shipping_lastname : 'Johnathan Doe' }}
                        </div>
                        <p class="confirm-address-text">
                            ...
                            @endif
                            {{ $order ? $order->shipping_city : 'San Francisco' }},
                            {{ $order ? $order->shipping_postcode : '94105' }}<br>
