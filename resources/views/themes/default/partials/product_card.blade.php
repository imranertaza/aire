<div class="product-card position-relative h-100">
    @php
        static $cachedShowCategoryBadge = null;
        if ($cachedShowCategoryBadge === null) {
            $cachedShowCategoryBadge = !empty($isFiltered) ||
                !empty($search) ||
                request()->hasAny([
                    'industry',
                    'building_type',
        ...
        $showCategoryBadge = isset($showCategoryBadge) ? $showCategoryBadge : $cachedShowCategoryBadge;
        ...
        $subtitle = $subtitle ?? ($product->description?->description ?? ($firstCategory?->category_name ?? 'ENTERPRISE FILTRATION'));

        static $cachedCart = null, $cachedCompare = null, $cachedFavorites = null;
        if ($cachedCart === null) {
            $cachedCart = session('cart', []);
            $cachedCompare = session()->get('compare_products', session()->get('compare', []));
            $cachedFavorites = session()->get('favorites', []);
        }
        $inCart = isset($cachedCart[$product->id]);
        $inCompare = in_array($product->id, $cachedCompare);
        $isFav = in_array($product->id, $cachedFavorites);

        $detailUrl = route('products.detail', $product->slug ?: $product->id);
        $targetUrl = $product->productLanding ? route('products.landing', $product->slug ?: $product->id) : $detailUrl;

        $price = (float) $product->price;
        $specialPrice = $product->special_price ?? ($product->special ? (float) $product->special->special_price : null);
        $hasSpecial = ($specialPrice !== null && $specialPrice < $price);
    @endphp
    ...
    <div class="product-card__image-wrapper">
        <a href="{{ $targetUrl }}">
            <img src="{{ $product->main_image ? getImageCacheUrl($product->main_image, 400, 400, 'webp') : theme_asset('img/Air-Purify.png') }}"
                ...
                alt="{{ $product->name }}" width="400" height="400" class="product-card__image" loading="lazy" decoding="async">
        </a>
    </div>
    ...
    <div class="product-card__actions">
        <div class="d-flex gap-2 align-items-center">
            ...
            <div class="d-flex flex-column gap-0">
                @if ($hasSpecial)
                    <span class="product-card__price pre">${{ number_format($price, 2) }}</span>
                    <span class="product-card__price">${{ number_format($specialPrice, 2) }}</span>
                @else
                    <span class="product-card__price">${{ number_format($price, 2) }}</span>
                @endif
            </div>
        </div>
        ...
        @else
            <a href="{{ $detailUrl }}" class="product-card__btn-learn">Learn more</a>
        @endif
    </div>
</div>
