                ...
                <div class="col-lg-9">
                    <div class="card border-light-subtle shadow-sm rounded-4 bg-white p-4">
                        <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                            <div>
                                <h4 class="fw-bold text-dark mb-1 fs-18"><i
                                        class="bi bi-journal-text text-primary me-2"></i>Account Ledger</h4>
                            ...
                            <div class="badge bg-primary-subtle text-primary rounded-pill px-3 py-2 fs-13 fw-semibold">
                                Balance: ${{ number_format($customer->balance ?? 0.0, 2) }}
                            </div>
                        </div>
                                    ...
                                    <tbody class="fs-14">
                                        @foreach ($ledgers as $item)
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($item->order_id)
                                                        <a href="{{ route('customer.orders.detail', $item->order_id) }}"
                                                            class="text-primary text-decoration-none fw-semibold">
                                                            #{{ $item->order_id }}
                                                        </a>
                                                        @elif($item->fund_request_id)
                                                        <span class="text-secondary">Fund Req
                                                            #{{ $item->fund_request_id }}</span>
                                                    @else
                                                ...
                                                <td>
                                                    @if ($item->transaction_type == 'Dr.')
                                                        <span
                                                            class="badge bg-danger-subtle text-danger rounded-pill px-2.5 py-1 fs-12">Debit</span>
                                                    @else
                                                        <span
                                                            class="badge bg-success-subtle text-success rounded-pill px-2.5 py-1 fs-12">Credit</span>
                                                    @endif
                                                </td>
                                                <td
                                                    class="fw-semibold {{ $item->transaction_type == 'Dr.' ? 'text-danger' : 'text-success' }}">
                                                    {{ $item->transaction_type == 'Dr.' ? '-' : '+' }}${{ number_format($item->amount, 2) }}
                                                </td>
                                                <td class="fw-bold text-dark">
                                                    ${{ number_format($item->rest_balance ?? 0.0, 2) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
