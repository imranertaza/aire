...
const submitForm = async () => {
    ...
    // 3. Images Tab Validation (Main image must be present either as existing preview or newly uploaded file)
    const hasMainImage = (mainImagePreviews.value && mainImagePreviews.value.length > 0) || (mainImageFile.value?.[0]?.file);
    if (!hasMainImage) {
