<template>
    <DashboardHeader title="Update Coupon" />
        ...
        <div class="container-fluid">
            <div class="row row-cols-1">
                <div class="card card-info">
                    ...
                    <form @submit.prevent="submitCoupon" v-if="!loading">
                        <div class="card-body">
                            <div class="row">
                                <!-- Left Column -->
                                <div class="col-md-6">
...
const fetchCoupon = async () => {
    try {
        const res = await axios.get(`/api/coupons/${route.params.id}`);
        const coupon = res.data.data;
        
        form.name = coupon.name;
        form.code = coupon.code;
        form.discount_type = coupon.discount_type.toString();
        form.discount_on = coupon.discount_on.toString();
        form.discount = coupon.discount;
        form.for_subscribed_user = coupon.for_subscribed_user.toString();
        form.for_registered_user = coupon.for_registered_user.toString();
        form.total_useable = coupon.total_useable;
        form.date_start = coupon.date_start;
        form.date_end = coupon.date_end;
        form.status = coupon.status.toString();
        
        if (coupon.categories) {
            form.categories = coupon.categories.map(c => c.id);
        }
        if (coupon.products) {
            form.products = coupon.products.map(p => p.id);
        }
        if (coupon.shipping_methods) {
            form.shipping_methods = coupon.shipping_methods.map(s => s.id);
        }
    } catch (error) {
