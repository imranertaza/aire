...
if (! function_exists('getSection')) {
    /**
     * Retrieve a dynamic CMS section from database by name.
     *
     * @param string $name
    ...
    function getSection(string $name, ?array $default = null): ?array
    {
        ...
        if (!isset($cachedSections[$name])) {
            try {
                $section = \App\Models\Section::where('name', $name)->first();
                $cachedSections[$name] = $section ? (is_array($section->data) ? $section->data : json_decode($section->data, true)) : null;
            } catch (\Throwable $e) {
                $cachedSections[$name] = null;
            }
        }
        return $cachedSections[$name] ?? $default;
    }
}
